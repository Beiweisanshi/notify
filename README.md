...
当前 MVP 以“后台服务先可交付”为边界：`agent-notify-tray` 目前是 Axum localhost 后台入口，先承担事件入口、通知触发、中文 Toast 文案、当前用户 Start Menu AppID/图标注册、原生托盘退出入口、`agent-notify://` 点击唤窗、hook 检查/修复和 HTTP focus 接口。完整 Tauri 托盘 UI、Toast 按钮、进程树精确校验、session 详情页和 `agentrun` 仍是后续计划。
...
assets/
  agent-notify-icon.svg
  agent-notify-icon.png
  agent-notify-icon.ico
src/
  agent-notify-core/
  hook-manager/
  agent-notify/
...
- `assets/agent-notify-icon.*`：当前 Windows Toast 和开始菜单快捷方式使用的应用图标资源。
...
- `src/agent-notify-tray/`：当前是本地 HTTP 后台入口，负责 session 状态、通知去重、Windows Toast、原生托盘退出菜单、`/focus/{sessionId}` 和点击通知后的 `/activate/{activationId}`；完整 Tauri 托盘壳尚未接入。
...
Windows Toast 还会写入当前用户开始菜单快捷方式和本地图标资源，用于让 Windows 为桌面程序分配可通知的中文 AppID 并显示自定义图标：

```text
%APPDATA%\Microsoft\Windows\Start Menu\Programs\智能任务通知.lnk
%LOCALAPPDATA%\AgentNotify\assets\agent-notify-icon.png
%LOCALAPPDATA%\AgentNotify\assets\agent-notify-icon.ico
```
...
MVP 当前面向 Windows。Tauri 是预留的常驻托盘外壳和通知运行时，但当前代码先交付后端服务能力：本地 HTTP、token 鉴权、事件解析、通知策略、hook 检查/修复、内存 session 表和一个可退出后台的原生托盘菜单。完整托盘 UI、session 面板、静音入口和“修复 hooks”按钮尚未实现。

当前 Windows Toast 展示中文标题、正文、详情和自定义图标，并在通知主体写入 `agent-notify://focus?activationId=...` deep link。`agent-notify-tray serve` 会创建或更新当前用户的 `智能任务通知.lnk`、注册 `agent-notify://` 协议，发送通知时通过 `Get-StartApps` 获取 `智能任务通知` 的 AppID；如果该 AppID 暂不可用，会降级尝试旧 `Agent Notify` 或 Windows PowerShell AppID，并把失败写入后台 stderr 日志。点击通知后，协议处理子命令会读取本地 token，调用 Bearer 鉴权的 `/activate/{activationId}`，后台用短期 activation 表映射回 session，再按 HWND、PID、父 PID、窗口标题顺序 best-effort 聚焦窗口。Bearer token 不得放进 URI。
...
## Desktop Tauri Floating UI

The visible desktop floating status bar is now the Tauri app `agent-notify-desktop`.
It owns the frameless always-on-top UI, drag position, hidden strip, `Ctrl + Shift + Space` restore, counters, completed notification chooser, and bar-anchored notification panel.

Use these package scripts for the Tauri UI:
...
Run the built desktop UI with:

```powershell
.\target\release\agent-notify-desktop.exe
```

When launched by Tauri, the backend is started as a headless process:

```powershell
agent-notify-tray serve --no-native-tray
```

`agent-notify-tray serve` still exists for the backend and Windows Toast fallback path, but the floating desktop UI should be tested through Tauri.
...
当前 MVP 支持 Windows Toast 主体点击唤窗：deep link 只能携带短期 activation nonce，不能携带 Bearer token。已实现的 `/focus/{sessionId}` 和 `/activate/{activationId}` 都是 Bearer 鉴权 HTTP 接口；聚焦按 HWND、PID、父 PID、窗口标题 best-effort 执行，Windows Terminal 多 tab 场景只承诺唤起窗口，不承诺切到具体 tab。Toast 按钮动作仍未实现。
