...
第一阶段不做完整终端替代品，只做“自动安装 hooks + 通用 hook + 本地通知后台 + 窗口唤起”的轻量系统。
...
Tauri 托盘应用启动或开启监听
        ...
        | 自动检查 / 安装 / 修复 hooks
        v
Claude Code hook / Codex hook / 通用命令 wrapper
        |
        | 统一 JSON 事件
        v
...
2. 事件来源层：Claude hook、Codex hook、普通长任务 wrapper。
3. 事件协议层：把不同工具的事件统一成同一种 JSON。
4. 通知后台层：本地常驻进程，负责接收事件、去重、展示通知、保存 session 状态。
5. 窗口控制层：根据 PID、窗口标题、HWND 等信息唤起对应窗口。
...
本项目必须实现 Hook Manager，让用户无需手工编辑 Claude/Codex 配置。Tauri 应用启动、监听开关从关闭变为开启、用户点击“修复 hooks”时，都会自动执行检查和安装。
...
每个备份需要记录原始路径、工具名、修改前 hash、修改后 hash、应用版本和 hook 版本。备份目录必须设置为当前 Windows 用户私有 ACL，并按数量或时间保留，例如每个工具最近 20 份或 30 天。
...
- 写入或校验失败：尝试恢复最近备份，状态为 `rollback_available` 或 `rollback_failed`。
...
Tauri 托盘应用的后台部分负责统一处理所有事件。文档中仍可把这个后台能力称为 `agent-notifyd`，但实现上它属于 Tauri 应用。
...
- Tauri 托盘应用。
- Windows Toast / Tauri notification plugin。
- Windows MVP 只承诺通知主体点击回调。
...
启动 Tauri 托盘应用。开机自启由用户自己配置，应用内提供监听开关。
...
- `config.json`、token 文件、备份目录必须设置为当前 Windows 用户私有 ACL。
...
- Codex 用户级配置备份、合并、校验
- Tauri UI 中的 hook 健康状态

验收：

- 全新机器只安装 Claude/Codex 后，启动 Tauri 应用即可看到 hook 自动安装结果。
- 监听开启时会重新检查 hooks。
- 删除 hook 配置后点击“修复 hooks”能恢复。
- 用户已有 hook 不被删除或覆盖。
...
- 能启动一个本地通知后台。
- 启动或开启监听时能自动检查 Claude/Codex hooks。
...
- 点击通知主体能唤起对应终端窗口；无法定位时打开 Tauri session 详情。
...
- 阶段 0 先单独验证。
- MVP 使用通知主体点击 + `agent-notify://focus?activationId=...` deep link。
