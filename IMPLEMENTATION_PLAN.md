...
实现一个 Windows 本地通知工具，用户可以从任意终端直接运行 `claude` 或 `codex`。当任务完成、失败、等待用户确认或等待继续输入时，系统弹出类似微信消息的气泡通知；点击通知后尽量唤起原终端窗口。
...
触发方式：hook -> agent-notify-hook.ps1 -> agent-notify emit --stdin -> Tauri 后台
通知实现：Tauri 托盘应用 + Windows Toast
点击唤窗：通知主体点击 -> agent-notify://focus -> best-effort 聚焦外部终端
Codex 边界：只支持带官方 hooks 的最新版 Codex
离线策略：后台离线、监听关闭、token 无效时直接丢弃事件
```
...
- `src/agent-notify-tray/`：Tauri 托盘应用、事件入口、通知和唤窗。
...
Hook Manager 是第一版的关键模块。它在三个时机运行：
...
- 写入或校验失败时，从最近备份回滚；回滚失败显示 `rollback_failed`。
...
备份目录和 token 文件必须设置为当前 Windows 用户私有 ACL。每个工具默认保留最近 20 份或 30 天。
...
第一版可以使用 localhost HTTP，后续再替换为 named pipe：
...
- Tauri 后台接收 `agent-notify emit --stdin`。
- 监听关闭时事件丢弃。
- token 无效时事件丢弃。
...
- Toast 主体点击 deep link 能聚焦窗口或打开 session 详情。
- 唤窗矩阵：PowerShell 独立窗口、CMD、Windows Terminal 单窗口、多 tab、定位失败。
...
- 点击通知能回到对应终端窗口或打开 session 详情。

## 里程碑

1. 验证 Toast 和窗口唤起。
2. 实现 Tauri 托盘壳和本地事件入口。
3. 实现 `agent-notify emit --stdin`。
4. 实现 Hook Manager 自动检查和安装。
5. 实现 Claude hook adapter。
6. 实现 Codex hook adapter。
7. 实现通知策略、去重和安全脱敏。
8. 完成端到端验收。
...
- Tauri 应用启动后能显示 Claude/Codex hook 健康状态。
- 缺失 hooks 时能自动安装。
- 安装前会备份用户配置。
- 从任意终端直接运行 `claude` 或 `codex` 都能触发 hook。
- 后台在线且监听开启时能弹 Windows 通知。
- 后台离线或监听关闭时事件直接丢弃。
- 点击通知能 best-effort 唤起原终端。
